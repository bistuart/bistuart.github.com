		...
		Public Sub Kesion()
			...
			Set KMRFObj = New Refresh
		          If KS.Setting(114)="" Then KS.Die "请先到""基本信息设置->模板绑定""进行模板绑定操作!"
				   ...
				   FileContent=Replace(FileContent,"{$PostBoardID}","")
				   Call GetList()
            ...
            End If
			 ...
			 if RSObj("purview")="1" Then
				 If KS.C("AdminName")<>"" or KS.C("UserName")=RSObj("UserName") Then
				  ListStr = ListStr & KS.CheckScript(KS.HtmlCode(RSObj("Memo")))
				 Else
				  ListStr = ListStr & "<div style=""width:350px;height:35px;line-height:30px;margin : 5px 20px; border : 1px solid #efefef; padding : 5px;background : #ffffee; line-height : normal;""><br/>对不起，此留言内容被留言者设定仅管理员可见！</div>"
				 End If
			 Else
				  ListStr = ListStr & KS.CheckScript(KS.HtmlCode(RSObj("Memo")))
			 End If
             ...
             ListStr = ListStr & "             <td><img src='../images/Face1/face" & rs(2) & ".gif' align='absmiddle'>&nbsp;<font color=red>" & KS.HtmlCode(RS(0)) & "</font><div align=right>回复时间：&nbsp;" & Rs(1) & "</div></td></tr>"
             End If
