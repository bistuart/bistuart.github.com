 ...
 if KS.chkclng(KS.S("isbest"))=1 then param=param & " and isbest=1 "
  ...
  Sub ShowTeamTopicList(rs)
   ...
   do while not rs.eof
	...
	%>
      <td><img src="images/topic.gif" align="absmiddle" />
...
do while not rs.eof 
	  dim rsu:set rsu=server.createobject("adodb.recordset")
	   ...
	   %>
		<ul>
		<li class="u1"><img src="<%=userfacesrc%>" border=0 width="<%=facewidth%>" height="<%=faceheight%>"></li>
	...
	End Function	
     ...
     '显示帖子列表
		function showtopic()
		 ...
		 response.write "    <tr>"
		 response.write "	  <td colspan=""2"" bgcolor=""#EDF5F9""><strong>"&rs("title")&"</strong></td>"
		 response.write "	</tr>"
		 response.write "	<tr>"
		 response.write "		<td width=""100"" align=""center"" bgcolor=""#FFFFFF"">"
		 response.write "			<img border=""0"" style=""border:#efefef 1px dashed; width:56px;height:56px"" src=""" & userfacesrc & """ /><br>"
		 response.write "			<a href=""../space/?" & rs(0) & """ target=""_blank"">" & rs(0) & "</a>"
		 response.write "		</td>"
		 response.write "		<td valign=""top"" bgcolor=""#FFFFFF"">发表于<span>" & rs("adddate") & "<br/><div class=""Content""> " & KS.HtmlCode(replace(rs("content"),"alt=","")) & "</div></td>"
		 response.write "	</tr>"
		 response.write "	<tr>"
...
Function ShowReplayContent(rsp)
  ...
  do while not rsp.eof
		  ...
		  if lcase(left(userfacesrc,4))<>"http" then userfacesrc=KS.GetDomain & userfacesrc
				 response.write "<table width=""99%"" cellpadding=""1"" cellspacing=""1"" bgcolor=""#efefef"">"
				 response.write "<tr>"
				 response.write " <td colspan=""2"" bgcolor=""#EDF5F9""><strong>" & rsp("title") & "</strong></td>"
				 response.write "	</tr>"
				 response.write "	<tr>"
				 response.write "		<td width=""100"" align=""center"" bgcolor=""#FFFFFF"">"
				 response.write "			<img style=""border:#efefef 1px dashed; width:56px;height:56px"" src=""" & userfacesrc & """ /><br /><a href=""?" & rsp(0) & """ target=""_blank"">" & rsp(0) & "</a></td>"
				 response.write "		<td valign=""top"" bgcolor=""#FFFFFF"">发表于<span>"&rsp("adddate") &"<div class=""Content"">" & rsp("content") & "</div></td>"
				 response.write "	</tr>"
				 response.write "	<tr>"
