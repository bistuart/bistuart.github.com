...
Sub Shop_ValidateCoupon()
  ...
  Dim RS:Set RS=Conn.Execute("SELECT Top 1 A.FaceValue,A.MinAmount,A.MaxDiscount,b.AvailableMoney,a.EndDate,a.status FROM KS_ShopCoupon A Inner Join KS_ShopCouponUser B On A.ID=B.CouponID Where B.CouponNum='" & CouponNum & "'")
  If Not RS.Eof Then
	   If DateDiff("s",RS("EndDate"),Now)>0 Then
		Response.Write "对不起,您输入的优惠券已过使用期限!"
	   ElseIf RS("Status")=0 Then
