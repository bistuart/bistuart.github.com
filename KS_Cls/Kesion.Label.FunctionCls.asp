...
Class RefreshFunction
				 ...
				 For I = 1 To Col
					  ...
					  select case PicStyle
                       ...
                       case 5  
						   ...
						   echoln "    <div class=""time"">时间：<span>" & KS.DateFormat(Node.SelectSingleNode("@adddate").text,1) & "</span></div>"
		...
		Function GetPriceStr(PriceType,Discount,Discount_v,Price_Original,Price,Price_Market,Price_Member)
		     ...
		     Select Case PriceType
			  Case 0:GetPriceStr="市场价："&Price_Market &"<br />商城价：￥"&Price &"<br />会员价：" & Price_Member
			  Case 1:GetPriceStr="原价："&Price_Original
			  Case 2:GetPriceStr="商城价：￥"&Price
			  Case 3:GetPriceStr="原　价："&Price_Original & "<br />会员价：" & Price_Member
			  Case 4:GetPriceStr="商城价：￥"&Price & "<br />会员价：" & Price_Member
			  Case 5:GetPriceStr="市场价："&Price_Market & "<br />商城价：￥"&Price
			  Case 6:GetPriceStr="市场价："&Price_Market &"<br />原　价："&Price_Original & "<br />会员价："&Price_Member
			  Case 7:GetPriceStr="市场价："&Price_Market &"<br />原　价："&Price_Original & "<br />商城价：￥"&Price & "<br />会员价："&Price_Member
			  Case 8:GetPriceStr="会员价："&Price_Member
			 End Select
			 If Not KS.IsNul(Discount) Then
				 If Cbool(Discount)=true Then 
				   If Discount_v = 10 Then
					GetPriceStr=GetPriceStr & "<br />折　扣：<span class=""discount"">0</span>"
				   Else
					GetPriceStr=GetPriceStr & "<br />折　扣：<span class=""discount"">"&Discount_v & "</span>折"
				   End If
				 End If
			 End If
		End Function
	  ...
	  End Function
	   ...
	   Function ExplainRollsLabelBody()
		...
		Function GetAnnounceList(LabelStyle)
			 ...
			 NaviStr       = KS.GetNavi(ParamNode.getAttribute("navtype"), ParamNode.getAttribute("nav"))
			 T_CssStr      = KS.GetCss(ParamNode.getAttribute("titleclss"))
			 T_Len         = KS.ChkClng(ParamNode.getAttribute("titlelen"))
		...
		Function GetLinkList(LabelStyle)
			 ...
			 Else
			   Para = Para & " And LinkType=" & LinkType & " Order BY OrderID"
			 End If
