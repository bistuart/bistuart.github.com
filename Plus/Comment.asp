...
<%
		...
		if C_Content="" Then 
		 ...
		 Response.End
		End if
		If Len(C_Content)>KS.C_S(ChannelID,14) and KS.C_S(ChannelID,14)<>0 Then
			 If Flag="NotAjax" Then
		...
		If KS.ChkClng(groupid)<>0 and Verific=1 Then
		  If KS.ChkClng(KS.U_S(GroupID,6))>0 Then
			 ...
			 If Not RS.Eof Then
			     ...
			     Call  KS.ScoreInOrOut(KS.C("UserName"),1,KS.ChkClng(KS.U_S(GroupID,6)),"系统","参与文档[<a href=""" & KS.GetItemUrl(channelid,rs(1),infoid,rs(2)) & """ target=""_blank"">" & RS(0) & "</a>]的评论!",1000,""&ChannelID&""&InfoID)
