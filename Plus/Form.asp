    ...
    End Sub
	 ...
	 Sub LoadSave()
	   ...
	   If (PostByStep=1 And Step=StepNum) Or PostByStep=0 Then
		   ...
		   IF Trim(KS.S("Verifycode"))<>Trim(Session("Verifycode")) And Shownum=1 then Set KSUser=Nothing:call KS.AlertHistory("验证码不正确！",-1):exit sub
	   End If
	   ...
	   rs.addnew
		...
		for k=0 to ubound(sql,2)
		 ...
		 Elseif sql(3,k)=9 Then
		     rs(sql(0,k))="<a href='" & KS.S(sql(0,k)) & "' target='_blank'>点击浏览</a>"
			 UpFiles=UpFiles&KS.S(sql(0,k))
		 Else
