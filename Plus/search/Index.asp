...
Class Link
	   ...
	   Sub ParseArea(sTokenName, sTemplate)
		...
		Sub SearchRecords()
		  ...
		  If Not KS.IsNul(RecordList) Then
			...
			For Each K in Rarr
			  Karr=Split(K,"@")
			  If Karr(0)<>"" Then
			   echo "<li><a href=""?m=" & Karr(2)&"&key=" & Karr(0) &"&stype=" & karr(3) & """>" & Karr(0) &" (结果:<font style='color:red'>" & KS.ChkClng(Karr(1)) & "</font>条)</a></li>"
			  End If
			Next
		  Else
		     echo "没有任何搜索记录!"
		...
		next
		If ChannelID=102 Then
		  echo "<li class=""curr""><a href=""?stype="&stype&"&key="&key &"&m=102"">问吧</a></li>"
		Else
		  echo "<li><a href=""?stype="&stype&"&key="&key &"&m=102"">问吧</a></li>"
		End If
		End Sub
