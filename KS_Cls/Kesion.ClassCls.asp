...
Class ClassCls
		...
		'添加频道或目录的过程
			...
			Sub GetAddChannelFolder(Action,FolderID, FormProcesPage)
			 ...
			 dim ShowADTF,AdParam,AdUrl,AdLinkUrl,AdP,AdType
			  ...
			  Else
				...
				WapFolderTemplateID="{@TemplateDir}/WAP专用模板/" & KS.C_S(ChannelID,1) & "/WAP频道首页.html"
				Else
				FolderTemplateID="{@TemplateDir}/" & KS.C_S(ChannelID,1) & "/栏目页.html"
				WapFolderTemplateID="{@TemplateDir}/WAP专用模板/" & KS.C_S(ChannelID,1) & "/WAP栏目页.html"
				End If
				TemplateID="{@TemplateDir}/" & KS.C_S(ChannelID,1) & "/内容页.html"
				WapTemplateID="{@TemplateDir}/WAP专用模板/" & KS.C_S(ChannelID,1) & "/WAP内容页.html"
			  End If
			...
			With Response
				...
				.Write "             <label id='add1'>"
				.Write "              &nbsp;<INPUT name='FolderName' onkeyup='ctoe()' type='text' value='" & FolderName & "' id='FolderName' title='请输入栏目名称' size=30><font color=red>*</font> 概括性的说明文字</label>"
				...
				.Write "            <td height='30' align='right'  width='200' class='clefttitle'><strong>" & vbCrLf
					   ...
					   If FolderID = "0" Then  .Write ("WAP栏目首页模板：")  Else  .Write ("WAP栏目模板：")
				...
				.Write "         <tbody id='temparea'>" & vbcrlf
					...
					.Write "    </td></tr>"
					 .Write "         <tr class=""tdbg"" onMouseOver=""this.className='tdbgmouseover'"" onMouseOut=""this.className='tdbg'"">" & vbCrLf
					 .Write "           <td height='30' align='right'  class='clefttitle' width='200'><strong>WAP内容页模板：</strong></td>" & vbCrLf
					 .Write "           <td height='28'>" & vbCrLf
					...
					.Write "     <td><b>&nbsp;<input name='FolderDomain' TYPE='text' value='" & FolderDomain & "' id='FolderDomain' class='upfile' size=30></b>&nbsp;如：http://news.kesion.com/，只对一级栏目有效 </td>" & vbCrLf
					.Write " </tr>" & vbCrLf
					.Write "</tbody>"
				...
				.Write "<td>"
				 ...
				 .Write "           <td height='28'> "
				 .Write "<input type='hidden' id='ClassContent' name='ClassContent' value='"& Server.HTMLEncode(ClassContent) &"' style='display:none' />"
				...
				Else
				.Write "            <td height='80' align='right' width='200' class='clefttitle'><div><strong>允许查看此栏目下供求信息联系方式的会员组：</strong></div><font color=blue>如果栏目是“认证栏目”，请在此设置允许查看此栏目下信息的联系方式的会员组,对应的供求内容页模板里需要放[KS_Charge]联系方式[/KS_Chagrge]才有效。</font></td>" & vbCrLf
				End If
				...
				.Write "            <td height='28'>&nbsp;<input name='ReadPoint' type='text' id='ReadPoint'  value='" & ReadPoint & "' size='6' class='upfile' style='text-align:center'>免费阅读请设为""<font color=red>0</font>""，否则有权限的会员阅读该栏目下的信息时将消耗相应点数，游客将无法阅读。"
				.Write "</td>"
				.Write "          </tr>" & vbCrLf
				.Write "          <tr class=""tdbg"" onMouseOver=""this.className='tdbgmouseover'"" onMouseOut=""this.className='tdbg'"">" & vbCrLf
				.Write "            <td height='60' align='right' width='200' class='clefttitle'><strong>默认与投稿者的分成比率：</strong></td>" & vbCrLf
				.Write "            <td height='28'>&nbsp;<input name='DividePercent' type='text' value='" & DividePercent & "' size='6' class='upfile' style='text-align:center'>% 系统将根据这里设置的分成比率将收成分给投稿者。建议设成10的整数倍!"
				.Write "</td>"
				.Write "          </tr>" & vbCrLf
				.Write "          <tr class=""tdbg"" onMouseOver=""this.className='tdbgmouseover'"" onMouseOut=""this.className='tdbg'"">" & vbCrLf
				...
				.write ">距离上次收费时间 <input name='PitchTime' type='text' class='upfile' value='" & PitchTime & "' size='8' maxlength='8' style='text-align:center'> 小时后重新收费<br>            &nbsp;<input name='ChargeType' type='radio' value='2'"
					 IF ChargeType=2 Then .Write " checked"
				.write ">会员重复阅信息 &nbsp;<input name='ReadTimes' type='text' class='upfile' value='" & ReadTimes & "' size='8' maxlength='8' style='text-align:center'> 页次后重新收费<br>            &nbsp;<input name='ChargeType' type='radio' value='3'"
				...
				.Write "   case 2:$('#editorarea').hide();$('#fsohtmlarea').hide();$('#classarea').html('链接地址：');$('#classtips').html('如 <font color=blue>http://www.kesion.com</font> 等');$('#templatearea').hide();$('#temparea').hide();break;" & vbcrlf
				.Write "   case 3:$('#editorarea').show();$('#fsohtmlarea').hide();$('#classarea').html('生成文件名：');$('#classtips').html('如 <font color=blue>about.html,intro.html,help.html</font>等');$('#templatearea').show();$('#temparea').hide();$('#channel').hide();$('#singlepage').html(""<iframe id='content___Frame' src='../KS_Editor/FCKeditor/editor/fckeditor.html?InstanceName=ClassContent&amp;Toolbar=NewsTool' width='600' height='400' frameborder='0' scrolling='no'></iframe>"");break;" & vbcrlf
				.Write " } }"&vbcrlf
